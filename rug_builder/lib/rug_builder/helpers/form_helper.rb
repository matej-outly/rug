			...
			def rug_form_for(object, options = {}, &block)
				...
				if options[:ajax] == true || options[:ajax].is_a?(Hash)
					...
					if options[:ajax].is_a?(Hash)
						...
						js_options += "clearOnSubmit: #{options[:ajax][:clear_on_submit] == true ? "true" : "false"}, " if options[:ajax][:clear_on_submit]
						...
						js_options += "showUrl: '#{options[:ajax][:show_url]}', " if options[:ajax][:show_url]
						js_options += "invisibleRecaptcha: #{options[:ajax][:invisible_recaptcha] == true ? "true" : "false"}, " if options[:ajax][:invisible_recaptcha]
						js_options += "logCallback: #{options[:ajax][:log_callback] == true ? "true" : "false"}, " if options[:ajax][:log_callback]
					end
					js_options += "}"
